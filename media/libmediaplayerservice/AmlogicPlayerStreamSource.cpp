...
{
    ALOGI("AmlogicPlayerStreamSource::open =[%s]", filename);
...
{
    ALOGI("AmlogicPlayerStreamSource Source_seek=%lld,whence=%d\n",pos,whence);
...
{
    ALOGI("AmlogicPlayerStreamSource Source_close");
#ifdef DUMP_DATA		
     if(dumpfd>=0)
		close(dumpfd);	
    dumpfd=-1;	 

    ...
    mDrmManagerClient(NULL)
{
	...
	if(mStopFeedingBufLevel<0.00001)
...
int HistoryMgt(const char * path,int r0w1,int mTime)
{
  ...
  ALOGV("History mgt old[%s,%d,%d]\n",static_path,0,lastplayingtime);
  ALOGV("History mgt    [%s,%d,%d]\n",path,r0w1,mTime);
  if(!r0w1)//read
  {
...
status_t AmlogicPlayer::exitAllThreads()
{
	...
	ALOGI("found %d not exit player threads,try exit it now\n",playerinfo.num);
	if(playerinfo.num>0)
	{
		int i;
...
AmlogicPlayer::SetCpuScalingOnAudio(float mul_audio){
	...
	if(val>0 && mul_audio >0){
		...
		ALOGV("set_cpu_freq_scaling_based_auido %d\n",val);
	}else{
		ALOGV("set_cpu_freq_scaling_based_auido failed\n");
	}
}

AmlogicPlayer::~AmlogicPlayer() {
    ALOGV("AmlogicPlayer destructor\n");
	Mutex::Autolock l(mMutex);
    release();
...
{	
	ALOGV("setDataSource,fd=%d,offset=%lld,len=%lld,not finished\n",fd,offset,length);
					...
					if(strstr(mTypeStr,"mpeg")!=NULL){/*mpeg,ts,ps\,may can't detect types here.*/
						...
						if(mSouceProtocol.get()!=NULL) {
							...
							if (mDecryptHandle == NULL) {
								ALOGE("after getDrmInfo, mDecryptHandle = NULL");
							}
							if (mDrmManagerClient == NULL) {
								ALOGE("after getDrmInfo, mDrmManagerClient = NULL");
							}		

							if (mDecryptHandle != NULL &&mDrmManagerClient !=NULL) {
								ALOGV("L%d:getmDecryptHandle", __LINE__);
								if (RightsStatus::RIGHTS_VALID != mDecryptHandle->status) {
						    		//notifyListener_l(MEDIA_ERROR, MEDIA_ERROR_UNKNOWN, ERROR_DRM_NO_LICENSE);
						    		ALOGE("L%d:getDrmInfo error", __LINE__);
								}else {
...
int AmlogicPlayer::UpdateProcess(int pid,player_info_t *info)
{
	...
	{
		ALOGV("Player status:%s, playback complete",player_status2str(info->status));
		if(mHttpWV == false) {
		if(!mEnded)
	...
	{
		ALOGV("Player status:%s, playback exit",player_status2str(info->status));
		mRunning=false;
		if(mHttpWV == false) {
	...
	else if(info->status==PLAYER_ERROR)
	{
		...
		//sendEvent(MEDIA_ERROR,MEDIA_ERROR_UNKNOWN,info->error_no);
		ALOGV("Player status:%s, error occur",player_status2str(info->status));
		//sendEvent(MEDIA_ERROR);
		sendEvent(MEDIA_ERROR,MEDIA_ERROR_UNKNOWN,-1);
		mState = STATE_ERROR;
		}
	}
	else if(info->status == PLAYER_HTTP_WV) {
		mHttpWV = true;
		sendEvent(0x12000);
		ALOGV("Get http wvm, goto WVM Extractor");	
	}else
	{
		...
		}else if(streaminfo_valied && mDuration>0&& info->bufed_pos>0 && mStreamInfo.stream_info.file_size>0)
		{
			...
			ALOGV("Playing percent on percent=%d,bufed pos=%lld,Duration=%lld\n",percent,info->bufed_pos,(mStreamInfo.stream_info.file_size));
		}
		else if(mDuration >0 && streaminfo_valied && mStreamInfo.stream_info.file_size>0)
		{
			...
			ALOGV("Playing buffer percent =%d\n",percent);
		}else
		{
...
status_t AmlogicPlayer::GetFileType(char **typestr,int *videos,int *audios)
{
	...
	ALOGV("GetFileType---Type=%s,videos=%d,audios=%d\n",mTypeStr,mhasVideo,mhasAudio);
	*typestr=mTypeStr;
...
status_t AmlogicPlayer::setdatasource(const char *path, int fd, int64_t offset, int64_t length, const KeyedVector<String8, String8> *headers)
{
	...
	else
	{
		...
		if(strncmp(path,"http",strlen("http"))==0 || 
			...
			mPlay_ctl.auto_buffing_enable=1;
			ALOGV("setDataSource enable buffering\n");
	}
...
status_t AmlogicPlayer::prepareAsync() {
	...
	if(mLowLevelBufMode){
		...
		mPlay_ctl.callback_fn.update_interval=100;
	}
	...
	streaminfo_valied=false;
	ALOGV("buffer level setting is:%f-%f-%f\n",
	...
	{
		ALOGV("Start player,check_url_type error %d!\n",check_is_playlist);
		return UNKNOWN_ERROR;
	}
	...
	{	
		...
		if(mPlayer_id>=0)
		{
			ALOGV("Start player,play_list_player ok\n");
			return NO_ERROR;
		}
	}
	else
...
status_t AmlogicPlayer::pause()
{
	...
	if(mhasVideo){/*video mode,and no video,no audio*/		
		if (mDecryptHandle != NULL) {      
	        mDrmManagerClient->setPlaybackStatus(mDecryptHandle,
	                Playback::PAUSE, 0);
	    }	
...
status_t AmlogicPlayer::updateMediaInfo(void)
{
	...
	if(ret!=0)
	{
		ALOGV("player_get_media_info failed\n");
		return NO_INIT;
	}
	...
	if(mStreamInfo.stream_info.bitrate>0 && mLowLevelBufMode)
	{
		...
		if(mStopFeedingBuf_ms>100){
			...
			ALOGI("recalculate the mStopFeedingBuf_ms=%dms,mStopFeedingBufLevel=%f,bufsize=%d\n",mStopFeedingBuf_ms,mStopFeedingBufLevel,bufsize);
...
{
	ALOGV("Set setVideoSurfaceTexture\n");
...
{	
	...
	switch(key)
	{
		case KEY_PARAMETER_AML_VIDEO_POSITION_INFO:
			...
			ALOGI("setParameter info to newrect=[%d,%d,%d,%d]\n",
			...
			ALOGI("setParameter info oldrect=[%d,%d,%d,%d]\n",
				left,top,right,bottom);
		...
		case KEY_PARAMETER_AML_PLAYER_SCREEN_MODE:
			if(mPlayer_id>=0){
				...
				ALOGI("setParameter screen mode = %d", mode);
	...
	if(fastNotifyMode){
		...
		*position=(int)(mStreamTime+(ALooper::GetNowUs()-mLastStreamTimeUpdateUS)/1000);/*jast let uplevel know,we are playing,they don't care it.(netflix's bug/)*/
		...
		if(!mPaused && LatestPlayerState==PLAYER_RUNNING){ 
			...
			//ALOGI(" getCurrentPosition mPlayTime=%d,mLastPlayTimeUpdateUS=%lld*1000,GetNowUs()=%lld*1000,realposition=%d\n",
	  ...
	  ALOGV("CurrentPosition=%dmS,mStreamTime=%d\n",*position,mStreamTime);
    	return NO_ERROR;
}

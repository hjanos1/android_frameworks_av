...
status_t AmSuperPlayer::setVideoSurfaceTexture(const sp<ISurfaceTexture>& surfaceTexture)
{
	TRACE();
	Mutex::Autolock l(mMutex);
	TRACE();
...
status_t    AmSuperPlayer::prepareAsync()
{
    ...
    if (mRenderTid > 0) {
        ALOGV("initThread(%d) started", mRenderTid);
    }
	return NO_ERROR;
}
...
void AmSuperPlayer::Notify(void* cookie, int msg, int ext1, int ext2,const Parcel *obj)
{
	...
	ALOGV("cookie=%p,msg=%x,ext1=%x,ext2=%x\n",cookie,msg,ext1,ext2);
	switch(msg){
			case MEDIA_NOP:	
				break;
...
player_type AmSuperPlayer::SuperGetPlayerType(char *type,int videos,int audios)
{
	...
	if(NULL!=mHardPara){
		if(match_codecs(type,mHardPara)){
			ALOGV("%s type will use hard-decoder,force-list:%s\n",type,mHardPara);
			return AMLOGIC_PLAYER;
		}
	}
	if(NULL!=mSoftPara&&match_codecs(type,mSoftPara)){
		ALOGV("%s type will use soft-decoder,force-list:%s\n",type,mSoftPara);
	...
	if(match_codecs(type,"m4a")) {
		...
		if (ret>0) {
			ALOGI("media.amsuperplayer.m4aplayer=%s\n",value);
        ...
        notify_callback_f notifyFunc)
{
    ...
    switch (playerType) {
		...
		case AMLOGIC_PLAYER:
            ...
            ALOGV("Create Amlogic Player");
            p = new AmlogicPlayer();
			#else
			ALOGV("Have not Buildin Amlogic Player Support");
			#endif
